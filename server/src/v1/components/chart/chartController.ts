...
const getFundamentalChartInformation = catchAsync(async (req, res) => {
  ...
  const sectors = sectorid
    ? (sectorid as string).split(",").map((id) => parseInt(id))
    : null;
  ...
  const sectorsData = sectors?.map((sector) => {
    return {
      ...
      datetime: queryResult.rows
        .filter((row: any) => row.sectorid === sector)
        .map((row: any) => row.datetime),
      price: queryResult.rows
        .filter((row: any) => row.sectorid === sector)
